<?php 
  ...
  if(!isset($_SESSION['user_data'])){
    ...
    $brandId = end($link_array);

    $profileData = $authController->getProfile();
    $brandData = $brandController->getBrand($brandId);
...
<html lang="en">
  ...
  <head>
    <title>Productos de la Marca Seleccionada | </title>
    <?php 
            ...
            <div class="row align-items-center">
              <div class="col-md-12">
                <ul class="breadcrumb">
                  ...
                  <li class="breadcrumb-item" aria-current="page">Productos de <?= htmlspecialchars($brand['name']) ?></li>
                </ul>
              </div>
              <div class="col-md-12">
                <div class="page-header-title">
                  <h2 class="mb-0">Productos de <?= htmlspecialchars($brand['name']) ?></h2>
                </div>
              </div>
            </div>
          </div>
                    ...
                    <div class="card product-card">
                      <div class="card-img-top">
                        <a href="<?= BASE_PATH ?>products/<?= htmlspecialchars($product['slug']) ?>">
                          <img src="<?= htmlspecialchars($product['cover'] ?? 'N/A') ?>" alt="image" class="img-prod img-fluid" />
                        </a>
                      </div>
                      <div class="card-body">
                        <a href="<?= BASE_PATH ?>products/<?= htmlspecialchars($product['slug']) ?>">
